...
celery.conf.beat_schedule = {
    ...
    'fetch-global-latest-updates-every-hour': {
...
def fetch_global_latest_updates_job():
    """
    Background job managed by Celery Beat.
    Fetches the latest global updates from all scrapers and adds them to the DB.
    """
    from app import create_app
        ...
        async def _fetch_and_store():
            for source, scraper in scrapers.items():
                ...
                for update in updates:
                    existing = db.session.get(Manga, update['id'])
                    if not existing:
                        manga = Manga(id=update['id'], title=update['title'], source=source, cover_url=update['cover_url'])
                        db.session.add(manga)
                    else:
                        existing.title = update['title']
                        if update['cover_url']:
                            existing.cover_url = update['cover_url']
                db.session.commit()
